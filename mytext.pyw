...
# -*- coding: utf-8 -*-
"""
MyText v3.2 - 极简强力文本编辑器 (Python/Tkinter)
功能：多标签页(ttk.Notebook) + 记事本全部功能 + 文本转换 + AES-256加密 + .MTEP密钥文件加密
中英文双语界面，Help菜单切换语言

v3.2 修复:
- 标签页系统从自定义 Canvas/Frame/Label 改为 ttk.Notebook（官方组件，100%兼容）
- 删除所有 Canvas、partial 闭包、bidi、tk.call encoding 等潜在崩溃点
- 关闭按钮用 Notebook tab 右键菜单 + 双击标签关闭（最兼容方案）
- 其余所有功能（加密/转换/主题/语言）一行未改
"""
...
class MyTextApp:
    ...
    def _build_ui(self):
        ...
        # ★ 核心：ttk.Notebook 做标签页（官方组件，稳定兼容）
